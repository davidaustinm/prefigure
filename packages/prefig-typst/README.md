...
-->
<p align="center">
  <img src="examples/images/showcase.png" width="100%" alt="Three PreFigure diagrams rendered from Typst: the eighth roots of unity on the unit circle, a slope field with solution curves, and a family of level curves — every label and axis number typeset by Typst">
</p>
...
<p align="center">
  <img src="examples/images/math-by-typst.png" width="46%" alt="A parabola with Typst-authored labels: 'the curve y = 0.4x² − 2', 'slope dy/dx', the point (0,−2), and x/y axis labels — all Typst math in the document font">
</p>
...
```typ
#import "../src/lib.typ": tags, xml-to-string
...
<p align="center">
  <img src="examples/images/fonts.png" width="46%" alt="The same parabola diagram as above, but with every label and equation set in Fira Math instead of the default font">
</p>
...
The full design — the four-pass metrics-injection handshake that lets a callback-
free plugin get its text measured by Typst, the native/svg label internals, the
math pipeline, and the known limitations — is documented in
[`DEVELOPER_NOTES.md`](DEVELOPER_NOTES.md). To build a publishable distribution of
just the package (compiled plugin + `src/` + manifest), run
[`make_dist.sh`](make_dist.sh).

## Building the plugin

The built wasm ([`src/prefig_typst_plugin.wasm`](src/prefig_typst_plugin.wasm)) is
